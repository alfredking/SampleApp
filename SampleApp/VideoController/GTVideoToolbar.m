...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    NSLog(@"super class is %@",[super class]); //self 应该指的是uiview
    if (self)
    {
        [self addSubview:({
            _avatorImageView = [[UIImageView alloc]initWithFrame:CGRectZero]; //-- equivalent to CGRectMake(0, 0, 0, 0)
            //view is layer's delegate layer应该是表层视图，masksToBounds可能是圆角的意思
...
-(void)layoutWithMode: (id)model
{
    //_avatorImageView.image = [UIImage imageNamed:@"icon.bundle/icon.png"];
    _avatorImageView.image = [UIImage imageNamed:@"icon.bundle/like@2x.png"];
    _nickLabel.text = @"极客时间";
    
    _commentImageView.image= [UIImage imageNamed:@"icon.bundle/like@2x.png"];
    _commentLabel.text = @"100";
    
    _likeImageView.image= [UIImage imageNamed:@"icon.bundle/like@2x.png"];
    _likeLabel.text = @"25";
    
    _shareImageView.image= [UIImage imageNamed:@"icon.bundle/like@2x.png"];
    _shareLabel.text = @"分享";
    ...
    NSString *vflString = @"H:|-15-[_avatorImageView]-0-[_nickLabel]->=0-[_commentImageView(==_avatorImageView)]-0-[_commentLabel]-15-[_likeImageView]-0-[_likeLabel]-15-[_shareImageView]-0-[_shareLabel]-15-|";
    [NSLayoutConstraint activateConstraints:[NSLayoutConstraint constraintsWithVisualFormat:vflString options:NSLayoutFormatAlignAllCenterY metrics:nil views:NSDictionaryOfVariableBindings(_avatorImageView,_nickLabel,_commentImageView,_commentLabel,_likeImageView,_likeLabel,_shareImageView,_shareLabel)]];//0 、15数值可变的时候使用metrics
